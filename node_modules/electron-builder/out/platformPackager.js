...
class PlatformPackager {
    ...
    archiveApp(format, appOutDir, outFile) {
        return __awaiter(this, void 0, void 0, function* () {
            const isMac = this.platform === metadata_1.Platform.MAC;
            return archive_1.archiveApp(this.devMetadata.build.compression, format, outFile, isMac ? path.join(appOutDir, `${ this.appInfo.productFilename }.app`) : appOutDir, isMac);
        });
    }
...
function getPublishConfigs(packager, platformSpecificBuildOptions) {
    ...
    if (publishers == null) {
        ...
        if (publishers == null && !util_1.isEmptyOrSpaces(process.env.GH_TOKEN)) {
            publishers = [{ provider: "github" }];
        }
        // if both tokens are set — still publish to github (because default publisher is github)
        if (publishers == null && !util_1.isEmptyOrSpaces(process.env.BT_TOKEN)) {
            publishers = [{ provider: "bintray" }];
        }
    }
    return util_1.asArray(publishers).map(it => typeof it === "string" ? { provider: it } : it);
}
exports.getPublishConfigs = getPublishConfigs;
function getResolvedPublishConfig(packager, publishConfig, errorIfCannot) {
    return __awaiter(this, void 0, void 0, function* () {
        function getInfo() {
            return __awaiter(this, void 0, void 0, function* () {
...
exports.getResolvedPublishConfig = getResolvedPublishConfig;
