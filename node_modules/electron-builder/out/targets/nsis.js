...
class NsisTarget extends platformPackager_1.TargetEx {
    ...
    computePublishConfig() {
        const publishConfigs = platformPackager_1.getPublishConfigs(this.packager, this.options);
        if (publishConfigs != null && publishConfigs.length > 0) {
            return platformPackager_1.getResolvedPublishConfig(this.packager.info, publishConfigs[0], true);
        } else {
    ...
    doBuild(appOutDir, arch) {
        return __awaiter(this, void 0, void 0, function* () {
            const publishConfig = yield this.publishConfig;
            if (publishConfig != null) {
                yield fs_extra_p_2.writeJson(path.join(appOutDir, "resources", ".app-update.json"), publishConfig);
            }
            const packager = this.packager;
            const archiveFile = path.join(this.outDir, `${ packager.appInfo.name }-${ packager.appInfo.version }-${ metadata_1.Arch[arch] }.nsis.7z`);
            return yield archive_1.archiveApp(packager.devMetadata.build.compression, "7z", archiveFile, appOutDir, false, true);
        });
    }
    finishBuild() {
